/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   parsing_check_bonus.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: lmerveil <lmerveil@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/13 13:38:45 by lmerveil          #+#    #+#             */
/*   Updated: 2024/03/14 13:32:18 by lmerveil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long_bonus.h"

void	ft_check_elements(t_data *data)
{
	int	x;
	int	y;

	y = -1;
	while (++y < data->height - 1)
	{
		x = -1;
		while (++x < data->width - 1)
		{
			if (data->grid[y][x] == 'P')
			{
				data->posx_p = x;
				data->posy_p = y;
				data->p++;
			}
			if (data->grid[y][x] == 'B' || data->grid[y][x] == 'Y'
				|| data->grid[y][x] == 'R' || data->grid[y][x] == 'K')
				data->enemy_count++;
			else if (data->grid[y][x] == 'C')
				data->c_count++;
			else if (data->grid[y][x] == 'E')
				data->e++;
		}
	}
	ft_parse_error(data);
}

int	ft_check_valid_elements(t_data data)
{
	int	y;
	int	x;

	y = -1;
	x = 0;
	while (++y < data.height - 1)
	{
		x = -1;
		while (++x < data.width - 1)
		{
			if (data.grid[y][x] != 'P' && data.grid[y][x] != 'C'
				&& data.grid[y][x] != 'E' && data.grid[y][x] != '0'
				&& data.grid[y][x] != '1' && data.grid[y][x] != 'B'
				&& data.grid[y][x] != 'R' && data.grid[y][x] != 'Y'
				&& data.grid[y][x] != 'K')
				return (0);
		}
	}
	return (1);
}

void	ft_parse_error(t_data *data)
{
	if (data->p != 1 || data->e != 1
		|| data->c_count <= 0 || !ft_check_valid_elements(*data))
		ft_printf("Error\n");
	if (!ft_check_valid_elements(*data))
		ft_printf("Non valid element in map!\n");
	if (data->p != 1)
		ft_printf("Map has no player!\n");
	if (data->e != 1)
		ft_printf("Map has no exit!\n");
	if (data->c_count == 0)
		ft_printf("Map has no collectibles!\n");
	if (data->p != 1 || data->e != 1
		|| data->c_count <= 0 || !ft_check_valid_elements(*data))
		pre_free_init_struct(data);
}

void	ft_parse_bonus(char *filename, t_data *data)
{
	check_fileformat(filename);
	is_not_folder(filename);
	init_struct(filename, data);
	get_dimensions_check_empty(data);
	check_rectangular(filename, data);
	fillgrid(filename, data);
	check_closed(data);
	ft_check_elements(data);
	data->tab_enemy = malloc(data->enemy_count * sizeof(t_enemy));
	if (!data->tab_enemy)
		pre_free_init_struct(data);
	flood_fill(data, data->posx_p, data->posy_p, data->parse_grid);
	if (!(data->c_flag == data->c_count && data->exit_flag == 1))
	{
		ft_printf("Error\nNo feasibe path to collectibles and exit!\n");
		free(data->tab_enemy);
		pre_free_init_struct(data);
	}
}
